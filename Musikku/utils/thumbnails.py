...
async def thumb_init(videoid):
    try:
        if os.path.isfile(f"Musikku/Cache/{videoid}.jpg"):
            return f"Musikku/Cache/{videoid}.jpg"
        ...
        if 1==1:
                ...
                async with session.get(f"http://img.youtube.com/vi/{videoid}/maxresdefault.jpg") as resp:
                    if resp.status == 200:
                        f = await aiofiles.open(
                            f"Musikku/Cache/thumb{videoid}.jpg", mode="wb"
                        )
                        await f.write(await resp.read())
                        await f.close()

            youtube = Image.open(f"Musikku/Cache/thumb{videoid}.jpg")
            ...
            image4.text((670, 150), "Musikku", fill="white", font = font2, stroke_width=2, stroke_fill="white", align ="left") 
            ...
            # description
            views = f"Penonton : {views}"
            duration = f"Durasi : {duration} Menit"
            ...
            image2.save(f"Musikku/Cache/{videoid}.jpg")
            file = f"Musikku/Cache/{videoid}.jpg"
